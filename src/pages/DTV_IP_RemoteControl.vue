  ...
  <div class="remote-control-container">
    <v-card class="remote-control-card" elevation="4">
      <v-card-title class="text-h6 text-center" style="color: white;">
        DirecTV IP Control for {{remoteName}}
      </v-card-title>
...
export default {
  ...
  computed: {
    ...
    unit() {
      const remoteNum = this.stateStore.remoteSelectedIndex
      return remoteNum   // 
    },
  ...
  methods: {
    numberBtnPressed(_number) {
      ...
      this.snackbarMessage = 'Command sent'
      this.snackbar = true
      console.log(`http://${this.dtvIPs[this.unit]}:8080/remote/processKey?key=${_button}`)
      fetch(`http://${this.dtvIPs[this.unit]}:8080/remote/processKey?key=${_button}`)
    },
    ...
    favoriteBtnPressed(_favorite) {
      ...
      this.snackbarMessage = 'Command sent'
      this.snackbar = true
      console.log(`http://${this.dtvIPs[this.unit]}:8080/tv/tune?major=${favCh}`)
      fetch(`http://${this.dtvIPs[this.unit]}:8080/tv/tune?major=${favCh}`)
    }
  }
}
</script>
